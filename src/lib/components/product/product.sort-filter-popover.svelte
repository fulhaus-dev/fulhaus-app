<script lang="ts">
	...
	import { ArrowDownWideNarrowIcon } from '@lucide/svelte';
	import { Popover } from 'bits-ui';
	import type { Snippet } from 'svelte';

	type ProductSortFilterPopoverProps = {
		open?: boolean;
		children: Snippet;
		hasSort: boolean;
	...
	let {
		...
		open = $bindable(false),
		aFilterIsOpen = false
	}: ProductSortFilterPopoverProps = $props();
</script>
...
<Popover.Root bind:open>
	<Popover.Trigger
		class={cn(
			...
			hasSort && 'bg-color-action-background text-color-action-text',

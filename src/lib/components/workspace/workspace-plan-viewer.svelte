...
<div class="w-fit space-y-2">
	{#each availableCreditPools as availableCreditPoolName (availableCreditPoolName)}
		<div class="flex items-center gap-x-2 text-base font-medium text-green-500">
			<BadgeCheckIcon />
			<p>{availableCreditPoolName} <span class="text-xs font-normal">Credits</span></p>
		</div>
	{/each}
</div>

{#if availableCreditPools?.length < 1}
